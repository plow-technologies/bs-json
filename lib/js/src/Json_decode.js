...
function tuple3(first, second, third, json) {
  if (Array.isArray(json)) {
    ...
    if (length === 3) {
      ...
      if (match.tag) {
        return /* Error */Block.__(1, ["Decoder for first item of tuple failed." + match[0]]);
      } else {
        var match$1 = Curry._1(second, json[1]);
        if (match$1.tag) {
          return /* Error */Block.__(1, ["Decoder for secondd item of tuple failed." + match$1[0]]);
        } else {
          var match$2 = Curry._1(third, json[2]);
          if (match$2.tag) {
            return /* Error */Block.__(1, ["Decoder for third item of tuple failed." + match$2[0]]);
          } else {
